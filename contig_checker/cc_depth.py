...
def load_dbdump(dbdump_fname, db_type="read"):
    """
    ...
    assert db_type in set(["read", "contig"]), "<db_type> must be 'read' or 'contig'."
    ...
    with open(dbdump_fname, 'r') as f:
        for line in f:
            ...
            if data[0] == "R":
                ...
                header = data[2]   # actually this is the first part of the header
                if header[0] == '>':   # trim header mark if remaining
                    header = header[1:]
            elif data[0] == "L":
                well, start, end = data[1:]
                if db_type == "read":
                    header = f"{header}/{well}/{start}_{end}"
...
def calc_depth(contig_db_prefix="CONTIGS",
    ...
    contigs = load_dbdump(f"{contig_db_prefix}.dbdump", db_type="contig")
    contigs.to_pickle("contigs.pkl")
    #contigs = pickle.load(open("contigs.pkl", 'rb'))

    reads = load_dbdump(f"{read_db_prefix}.dbdump", db_type="read")
